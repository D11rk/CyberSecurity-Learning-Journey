...
To complete the RootMe room, the following steps were performed:

- Scanned the machine using Nmap
- Enumerated directories using Gobuster
- Identified a file upload bypass using .phtml
- Uploaded a php5 reverse shell with the correct VPN IP
- Caught the shell using netcat
- Enumerated SUID binaries
- Used the Python SUID exploit from GTFOBins to escalate to root
- Retrieved both user and root flags

This room demonstrates a straightforward exploitation chain combining web vulnerabilities with privilege escalation.
